...
const CongregationUserRole = ({ member }) => {
	...
	const [roles, setRoles] = useState(member.role);
	const [tmpRoles, setTmpRoles] = useState(member.role);
	...
	useEffect(() => {
		...
		for (let i = 0; i < roles.length; i++) {
			if (roles[i] === 'admin') {
				...
				setIsLMMO(true);
			} else if (roles[i] === 'view-schedule-meeting') {
	...
	useEffect(() => {
		...
		if (isViewMeetingSched) {
			newRoles.push('view-schedule-meeting');
		}
	...
	useEffect(() => {
		setRoles(member.role);
		setTmpRoles(member.role);
	}, [member]);
