...
const AnnouncementItemDetails = () => {
	...
	useEffect(() => {
		...
		if (currentData) {
			...
			if (language) {
				...
				setBodyContent(currentData[language]?.content || '<p></p>');
			}
		}
	...
	useEffect(() => {
		...
		if (language) {
			...
			currentData[language].title = announcementTitle;
			currentData[language].desc = announcementDesc;
			currentData[language].content = bodyContent;
		}
